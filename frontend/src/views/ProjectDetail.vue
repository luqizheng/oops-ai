<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center">
            <el-button @click="router.go(-1)" link :icon="ArrowLeft" class="mr-4" />
            <div>
              <h1 class="text-xl font-bold text-gray-900 dark:text-white">{{ project.name }}</h1>
              <p class="text-sm text-gray-500 dark:text-gray-400">项目标识: {{ project.key }}</p>
            </div>
          </div>
          <div class="flex items-center space-x-3">
            <el-button @click="router.push(`/projects/${project.id}/edit`)" :icon="Edit" size="small" plain>
              编辑项目
            </el-button>
            <el-button @click="manageMembers" :icon="User" size="small" type="primary">
              管理成员
            </el-button>
          </div>
        </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <div>
              <el-tag :type="projectStatusType" size="large" effect="dark">{{ project.status }}</el-tag>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">项目描述</p>
                <p class="text-gray-900 dark:text-white">{{ project.description || '暂无描述' }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">创建时间</p>
                <p class="text-gray-900 dark:text-white">{{ formatDate(project.createdAt) }}</p>
              </div>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">最后更新</p>
                <p class="text-gray-900 dark:text-white">{{ formatDate(project.updatedAt) }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">项目成员</p>
                <div class="flex -space-x-2 overflow-hidden">
                  <div v-for="(member, idx) in project.members.slice(0, 5)" :key="idx" 
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">项目统计</h2>
          <div class="space-y-4">
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">总需求数</span>
              <span class="text-xl font-bold text-gray-900 dark:text-white">{{ project.requirements.length }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">进行中需求</span>
              <span class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ inProgressRequirements }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">已完成需求</span>
              <span class="text-xl font-bold text-green-600 dark:text-green-400">{{ completedRequirements }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">延期需求</span>
              <span class="text-xl font-bold text-red-600 dark:text-red-400">{{ overdueRequirements }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white">项目进度</h2>
        </div>
        <div class="space-y-6">
          <!-- 进度条 -->
          <div>
            <div class="flex justify-between items-center mb-2">
              <span class="text-sm text-gray-500 dark:text-gray-400">整体进度</span>
          ...
          <!-- 状态分布 -->
          <div>
            <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">需求状态分布</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div v-for="status in requirementStatuses" :key="status.value" 
                class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-500 dark:text-gray-400">{{ status.label }}</span>
                  <span class="text-lg font-semibold text-gray-900 dark:text-white">{{ status.count }}</span>
                </div>
                <el-progress :percentage="status.percentage" :stroke-width="6" :color="status.color" class="mt-2" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">需求列表</h2>
            <el-button @click="router.push(`/requirements/new?projectId=${project.id}`)" type="primary" size="small">
          ...
          >
            <el-table-column prop="title" label="需求名称" min-width="200">
              <template #default="{ row }">
                <div class="font-medium text-gray-900 dark:text-white hover:text-primary-600 dark:hover:text-primary-400 cursor-pointer">
                  {{ row.title }}
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="负责人" width="120">
              <template #default="{ row }">
                <span class="text-gray-600 dark:text-gray-300">{{ row.assignee?.name || '未分配' }}</span>
              </template>
            </el-table-column>
            <el-table-column label="截止日期" width="140">
              <template #default="{ row }">
                <span :class="row.dueDate && new Date(row.dueDate) < new Date() ? 'text-red-500' : 'text-gray-600 dark:text-gray-300'">
            ...
            <el-table-column label="操作" width="100" fixed="right">
              <template #default="{ row }">
                <el-button @click="router.push(`/requirements/${row.id}`)" link size="small">
                  查看
                </el-button>
              </template>
            </el-table-column>
          ...
          <div v-if="project.requirements.length === 0" class="text-center py-12">
            <el-empty description="暂无需求" :image-size="120">
              <template #extra>
                <el-button @click="router.push(`/projects/${project.id}/requirements/new`)" type="primary" plain>
                  立即创建
                </el-button>
              </template>
            </el-empty>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">团队成员</h2>
            <el-button @click="manageMembers" link size="small" :icon="CirclePlus">
              添加成员
            </el-button>
          </div>
          
          <div class="space-y-4">
            <div v-for="member in project.members" :key="member.userId" 
              ...
              <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-600 flex items-center justify-center">
                  <span class="text-sm font-bold text-slate-500 dark:text-slate-300">{{ member.user?.name?.charAt(0) || 'U' }}</span>
                </div>
                <div>
                  <div class="font-medium text-gray-900 dark:text-white">{{ member.user?.name || '未知用户' }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">{{ member.user?.email }}</div>
                </div>
              </div>
              <el-tag :type="getRoleType(member.role)" size="small" effect="plain">{{ getRoleLabel(member.role) }}</el-tag>
            </div>
          </div>
          
          <div v-if="project.members.length === 0" class="text-center py-12">
            <p class="text-gray-500 dark:text-gray-400">暂无团队成员</p>
            <p class="text-sm text-gray-400 dark:text-gray-500 mt-2">点击上方按钮添加成员</p>
          </div>
        </div>
      </div>
    </main>
    ...
    <!-- 成员管理侧边抽屉 -->
    <el-drawer v-model="showMembersDrawer" title="" size="650px" direction="rtl" :with-header="false" class="modern-drawer">
      <div class="p-6">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-gray-900 dark:text-white">
            成员管理 - {{ project.name }}
          </h3>
          <el-button @click="closeMembersDrawer" size="small" text :icon="CircleClose" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200" />
        </div>

        <!-- 添加成员表单 -->
        <div class="bg-slate-50 dark:bg-gray-700 p-6 rounded-2xl border border-slate-100 dark:border-gray-600 mb-8">
          <h4 class="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-4">录入新成员</h4>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <el-button @click="addProjectMember" :disabled="!newMember.userId || !newMember.role" type="primary" class="rounded-lg h-11">
              添加
            </el-button>
          </div>
        </div>
        ...
        <div v-if="project.members.length > 0" class="space-y-4">
          <div v-for="member in project.members" :key="member.userId"
            ...
            <div class="flex justify-between items-center">
              ...
              <div class="flex items-center space-x-3">
                <el-select v-model="member.role" @change="updateProjectMember(member)" size="small" class="w-36 rounded-lg" style="width:100px">
                  <el-option label="开发工程师" value="developer" />
                  <el-option label="测试工程师" value="tester" />
                  <el-option label="产品经理" value="product_manager" />
                  <el-option label="项目经理" value="project_manager" />
        ...
        <div v-else class="text-center py-12">
          ...
          <p class="text-sm text-gray-400 dark:text-gray-500 mt-2">使用上方表单添加成员</p>
        </div>
      </div>
    </el-drawer>
    ...
    <div v-if="actionFeedback.show" :class="[
      ...
      <div class="flex items-center justify-between">
        ...
        <el-button v-if="actionFeedback.undoable" link @click="handleUndo" class="text-primary-600 dark:text-primary-400">
          撤销
        </el-button>
      </div>
    </div>
...
const inProgressRequirements = computed(() => {
  return project.value.requirements.filter(req => 
    req.status === 'developing' || req.status === 'testing' || req.status === 'reviewing'
  ).length
})

const completedRequirements = computed(() => {
  return project.value.requirements.filter(req => req.status === 'done').length
})
...
const requirementStatuses = computed(() => {
  ...
  return statuses.map(status => {
    const count = project.value.requirements.filter(req => req.status === status.value).length
    const percentage = project.value.requirements.length > 0 ? 
      Math.round((count / project.value.requirements.length) * 100) : 0
...
const addProjectMember = async () => {
  if (!project.value || !newMember.value.userId || !newMember.value.role) return
...
const updateProjectMember = async (member: ProjectMember) => {
  ...
  try {
    await axios.put(
      `/projects/${project.value.id}/members/${member.userId}`,
      { role: member.role }
    )
    // 重新获取项目详情以更新成员列表
    await fetchProjectDetail()
...
const removeProjectMember = async (userId: string) => {
  ...
  // 先备份要移除的成员信息，用于撤销操作
  const memberToRemove = project.value.members.find(member => member.userId === userId)
  if (!memberToRemove) return

  // 乐观更新UI
  const index = project.value.members.findIndex(member => member.userId === userId)
  if (index > -1) {
