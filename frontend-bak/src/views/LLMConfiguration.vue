        ...
        >
          ...
          <div class="space-y-2 text-sm text-gray-600 mt-4">
            ...
            <div>
              ...
              <span
                :class="config.isActive ? 'text-green-600' : 'text-red-600'"
              >
    ...
    >
      <div class="p-6">
        ...
        <el-form :model="currentConfig" label-width="80px" class="space-y-5">
          ...
          <el-form-item label="API端点">
            <el-input
              ...
              :placeholder="
                getEndpointPlaceholder(currentConfig.provider || 'openai')
              "
              class="rounded-lg"
              size="large"
            />
          ...
          <el-form-item
            label="API密钥"
            v-if="currentConfig.provider !== 'local'"
          >
            <el-input
          ...
          <div class="flex items-center space-x-6">
            <el-checkbox v-model="currentConfig.isDefault"
              >设为默认配置</el-checkbox
            >
        ...
        <div class="mt-8 flex justify-end space-x-3">
          <el-button @click="closeDrawer" class="rounded-lg"> 取消 </el-button>
          <el-button
...
const updateConfig = async () => {
  ...
  try {
    await axios.put(
      `/llm/configurations/${currentConfig.value.id}`,
      currentConfig.value,
    );
    closeDrawer();
    fetchConfigurations();
...
const deleteConfig = async (id: string) => {
  // 先备份要删除的配置，用于撤销操作
  const configToDelete = configurations.value.find(
    (config) => config.id === id,
  );
...
const testConnection = async (config: LLMConfig) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      showFeedback(
        "连接测试失败：" + (response.data.message || "未知错误"),
        "error",
      );
    }
  } catch (error) {
