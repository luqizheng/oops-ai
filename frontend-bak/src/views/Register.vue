<template>
  <div
    class="min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800"
  >
    <!-- 左侧品牌展示区 -->
    ...
    <div class="flex w-full md:w-1/2 items-center justify-center p-8">
      <div
        class="max-w-md w-full space-y-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 md:p-12 animate-fadeIn"
      >
        <div>
          <h2
            class="mt-6 text-center text-2xl md:text-3xl font-bold text-gray-900 dark:text-white"
          >
            创建新账号
          </h2>
...
const handleRegister = async () => {
  ...
  try {
    ...
    errorMessage.value =
      error.response?.data?.message || "注册失败，请检查信息是否正确";
  } finally {
