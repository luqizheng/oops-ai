...
export class ProjectsService {
  ...
  async updateProject(projectId: string, submit: UpdateProjectSubmit): Promise<UpdateProjectResult> {
  ...
  async addMember(projectId: string, submit: AddProjectMemberSubmit): Promise<ProjectMemberListItem> {
    const project = await this.prisma.project.findUnique({ where: { id: projectId } });
    if (!project) {
      throw new NotFoundException(`Project with ID ${projectId} not found`);
    }
